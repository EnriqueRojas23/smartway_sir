    ...
    namespace Dapper
    {
        ...
        public CancellationToken CancellationToken { get { return cancellationToken; } }
            ...
            static Action<IDbCommand> GetInit(Type commandType)
            {
                ...
                var bindByName = GetBasicProperSmartwayetter(commandType, "BindByName", typeof(bool));
                var initialLongFetchSize = GetBasicProperSmartwayetter(commandType, "InitialLONGFetchSize", typeof(int));
            ...
            static MethodInfo GetBasicProperSmartwayetter(Type declaringType, string name, Type expectedType)
            {
...
#pragma warning restore 618
                        ...
                        if (specializedConstructor == null)
                        {
                            ...
                            if (item.Property != null)
                            {
                                if (type.IsValueType())
                                {
                                    il.Emit(OpCodes.Call, DefaultTypeMap.GetProperSmartwayetter(item.Property, type)); // stack is now [target]
                                }
                                else
                                {
                                    il.Emit(OpCodes.Callvirt, DefaultTypeMap.GetProperSmartwayetter(item.Property, type)); // stack is now [target]
                                }
                            }
                            else
                            {
...
#pragma warning restore 612, 618
        ...
        static bool IsParameterMatch(ParameterInfo[] x, ParameterInfo[] y)
        {
...
#endif
            internal static MethodInfo GetProperSmartwayetter(PropertyInfo propertyInfo, Type type)
            {
            ...
            internal static List<PropertyInfo> GetSettableProps(Type t)
            {
                return t
                      ...
                      .Where(p => GetProperSmartwayetter(p, t) != null)
                      .ToList();
            }
        ...
        {
            private readonly Type _type;
            private readonly Func<Type, string, PropertyInfo> _properSmartwayelector;

            /// <summary>
            /// Creates custom property mapping
            /// </summary>
            /// <param name="type">Target entity type</param>
            /// <param name="properSmartwayelector">Property selector based on target type and DataReader column name</param>
            public CustomPropertyTypeMap(Type type, Func<Type, string, PropertyInfo> properSmartwayelector)
            {
                if (type == null)
                    throw new ArgumentNullException("type");

                if (properSmartwayelector == null)
                    throw new ArgumentNullException("properSmartwayelector");

                _type = type;
                _properSmartwayelector = properSmartwayelector;
            }
            ...
            public SqlMapper.IMemberMap GetMember(string columnName)
            {
                var prop = _properSmartwayelector(_type, columnName);
