...
function btnBuscarPersona_onclick(obj, event) {
    ...
    var documento = $('#cli_str_documento').val();

    if (documento == "")
    {
        swal({ title: "¡Error!", text: "¡Ingresar N° de documento a buscar!", type: "error", confirmButtonText: "Aceptar" });
    }
    else
    {
...
function btnBuscarDocumento_onclick(obj, event) {
    ...
    var documento = $('#tdoc_str_numero').val();
...
function CargarGrillaDetalle()
{
    ...
    oDetalleTable = $('.dataTables-Detalle').DataTable({
        ...
        "columns": [
                ...
                {
                    "title": "Garantía", "data": "garantia", "name": "garantia", "class": "text-center",
                     "mRender":
                     function (data, type, full)
                     {
                         if (full.garantia_activa == "SI")
                         {
                 ...
                 {
                     "title": "Dentro Garantía", "data": "garantia_activa", "name": "garantia_activa", "class": "text-center", "sWidth": "10%",
                     "mRender":
                     function (data, type, full)
                     {
                         if (full.garantia_activa == "SI")
...
function ValidarImei()
{
    ...
    if (Imei == "" || Item == "")
    {
        ...
        var url = $('#txtImeiValidar').data("url") + "?co_item=" + encodeURIComponent(Item) + "&imei=" + Imei;
        $.ajax(
                {
                    type: "POST",
                    async: true,
...
function OnCompleteTransaction_RegistrarIncidencia(xhr, status)
{
    ...
    if (jsonres.res == true)
    {
        if (jsonres.mensaje_adicional == true)
        {
            swal({ title: "Información", text: jsonres.MensajeAdic, type: "info", closeOnConfirm: false },
                    function ()
                    {
                    swal({
