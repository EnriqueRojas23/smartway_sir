...
const $grilla = $("#griddiagnostico");
const $pagergrilla = $("#griddiagnosticopager");
...
function reload()
{
    var idcat =  $('#idcategoriareparacion').val()
    var idfabr =  $('#idfabricante').val()
    var vdataurl = $grilla.data("dataurl")  + "?idcat=" + idcat + "&idfar=" + idfabr
...
function configurarGrilla()
{
    ...
    var idcat =  $('#idcategoriareparacion').val()
    var idfar =  $('#idfabricante').val()
    
    var vdataurl = $grilla.data("dataurl")  + "?idcat=" + idcat +"&idfar=" + idfar  
    ...
    $grilla.jqGrid({
        ...
        datatype: 'json',
        mtype: 'Get',
        colNames: ['', 'Código Smartway', 'Diagnóstico' ,'Categoría Reparación' ,  'Acciones'],
        colModel:
        [
            { key: true, hidden: true, name: 'iddiagnostico', index: 'iddiagnostico' ,classes:"grid-col"},

             { key: false, hidden: false, editable: true ,name: 'codigosmartway'
                 , index: 'codigosmartway', width: '100px', align: 'left'
                 , classes:"grid-col",formatter: formatedit
             },
             { key :false , hidden:false, editable: true , name :'descripcion' 
               , index : 'descripcion', width: '500px', formatter: formatedit , classes:"grid-col"
             },

            { key: false, hidden: false, editable: true ,name: 'categoriareparacion'
                    , index: 'categoriareparacion', width: '190px',editrules: { required: true}, align: 'center'
                    , classes:"grid-col",formatter: formatedit
                    , edittype: "select"
                    , editoptions: { dataUrl: fcnUrlControlGrid('categoriareparacion')}
            },
            //  { key: false, hidden: false, editable: true ,name: 'tipodiagnostico'
            //         , index: 'tipodiagnostico', width: '140px', align: 'left'
            //         , classes:"grid-col",formatter: formatedit
            //         , edittype: "select"
            //         , editoptions: { dataUrl: fcnUrlControlGrid('tipodiagnostico')}
            // },
            // { key: false, hidden: false, editable: true ,name: 'tipoproducto'
            //         , index: 'tipoproducto', width: '130px', align: 'left'
            //         , classes:"grid-col",formatter: formatedit
            //         , edittype: "select"
            ...
            //         , index: 'fabricante', width: '100px', align: 'left'
            //         , classes:"grid-col",formatter: formatedit
            //         , edittype: "select"
         ...
         { key: false, hidden: false, editable: false ,name: 'iddiagnostico', width:'140' , index: 'iddiagnostico' ,  formatter:  displayButtons,classes:"grid-col"}
        ],
        pager: $pagergrilla,
        rowNum: 40,
...
{
        var duplicar = '<button type="button" title="duplicar" class="btn btn-warning btn-xs btn-outline" onclick="irduplicar(' + cellvalue + ')"><i class="fa fa-copy"></i></button>';
        var vincularproducto = '<button title="Vincular" type="button" class="btn btn-warning btn-xs btn-outline" onclick="irvincular(' + cellvalue + ')"><i class="fa fa-book"></i></button>';
...
        return vincularproducto + duplicar + editar + guardar + control + restore;
}
var editOptionsNew = {
        keys: true,
...
function irvincular(id)
{
     
     let url = UrlHelper.Action("VincularTipoProductoModal", "Mantenimiento", "Mantenimiento") + "?iddiagnostico=" + id;

     $.get(url , function(data) {
        $("#modalcontent").html(data)
        $("#modalcontainer").modal("show")

         var dlbAccesoriosODS = $('select[name="TipoProductosSeleccionados"]').bootstrapDualListbox({
...
function OnCompleteTransaction_Vincular(xhr, status)
{
